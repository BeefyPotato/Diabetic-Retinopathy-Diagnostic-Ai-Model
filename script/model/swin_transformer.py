...
existing_model_path = "swin_small_patch4_window7_224_22k.pth"
class Mlp(nn.Module):
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, fused_window_process=False, existing_model_path=None, model_size="large", **kwargs):
        super().__init__()
        print("start")
        ...
        # Reinitialize the final head layer
        self.head = nn.Linear(self.num_features, self.num_classes)  # 5 classes
        # Calculate 'a' for uniform initialization
        a = 1 / math.sqrt(self.num_features)
        # Apply uniform initialization
        nn.init.uniform_(self.head.weight, -a, a)
        if self.head.bias is not None:
            nn.init.zeros_(self.head.bias)
        if existing_model_path is None:
            print("loading existing state_dict")
            state_dict = torch.load(f"/mnt/database/swin_{model_size}_patch4_window7_224_22k.pth")
            # Check if the weights are wrapped under a 'model' key
